...
route.get("/post/:id", (req, res) => {
  ...
  if (postsData.length === 0)
    return res.send(401).send({ error: "no posts found" });
  const parsedPostsData = JSON.parse(postsData);
  const requiredPost = parsedPostsData.posts.filter(
    (post) => post.id === req.params.id
  );

  if (requiredPost.length === 0)
    return res.send(400).send({ error: "no post found" });
...
route.post("/post/:postId/comment", (req, res) => {
  ...
  const newArray = parsedPostsContent.posts.filter(
  ...
  fs.writeFile(
    "model/posts.json",
    JSON.stringify(parsedPostsContent),
    (err) => {
      if (err) {
